...
class TimestampObsAccumulator:
    ...
    def put(self, data: Dict[str, np.ndarray], timestamps: np.ndarray, additional_data: List[Dict[str, np.ndarray]]=None, additional_timestamps:List[np.ndarray]=None):
        """
        data:
            key: T,*
        """
        ...
        if (additional_data is not None):
            assert (additional_timestamps is not None)
            assert len(additional_data) == len(additional_timestamps)

            additional_local_idxss = []
            for i , a in enumerate(additional_timestamps):
                this_local_idxs, this_global_idxs, this_next_global_idx = get_accumulate_timestamp_idxs(
                    timestamps=additional_timestamps[i],
                    start_time=self.start_time,
                    dt=self.dt,
                    eps=self.eps,
                    next_global_idx=self.next_global_idx
                )
                assert this_global_idxs == global_idxs
                assert this_next_global_idx == next_global_idx
                assert len(this_local_idxs) == len(local_idxs)
                additional_local_idxss.append(this_local_idxs)
        ...
        if len(global_idxs) > 0:
            if self.timestamp_buffer is None:
                ...
                for i, this_data in enumerate(additional_data):
            ...
            for key, value in data.items():
                self.obs_buffer[key][global_idxs] = data[key][local_idxs]
            for i, this_data in enumerate(additional_data):
                for key,value in this_data.items():
                    self.obs_buffer[key][global_idxs] = this_data[key][additional_local_idxss[i]]
