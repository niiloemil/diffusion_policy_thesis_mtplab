...
class RTDEInterpolationController(mp.Process):
    ...
    def schedule_waypoint(self, pose, target_time):
        t=time.time()
        if not (target_time > t):
            warnings.warn("Warning. Tried to submit action with timestamp < current time. Skipping this action")
            return False
        pose = np.array(pose)
        assert pose.shape == (6,)
    ...
    def moveJ(self, target_joints, vel=1.05, accel = 1.4, wait=True):
        assert self.is_alive()
        ...
        self.input_queue.put(message)
        if wait:
            self.move_done_event.wait(10)
            assert self.is_alive()
        self.move_done_event.clear()
        return True
    
    def moveL(self, target_pose, vel=1.05, accel = 1.4, wait=True):
        assert self.is_alive()
        target_pose = np.array(target_pose)
        assert target_pose.shape == (6,)

        message = {
            'cmd': Command.MOVEL.value,
            ...
            while keep_running:
                ...
                for i in range(n_cmd):
                    ...
                    if cmd == Command.STOP.value:
                        ...
                        #ENTER JOINT MOVEMENT MODE. TAKE PARAMS FROM BUFFER. REMOVE FUTURE SERVO VALUES FROM BUFFER. UPDATE ROBOT POSE AT MOVEMENT END. 

                        target_joints = command['target_pose']
                        vel = command['vel']
                        accel = command['accel']
                        #print(target_pose, vel, accel)
                        rtde_c.servoStop()
                        assert rtde_c.moveJ(target_joints, vel, accel)


                        curr_pose = rtde_r.getActualTCPPose()
                        #print("actual",curr_pose)
                        ...
                        for key in self.receive_keys:
                            state[key] = np.array(getattr(rtde_r, 'get'+key)())
                        state['robot_receive_timestamp'] = time.time()
                        #print("target tcp pose:",state['TargetTCPPose'])
                        ...
                        #print(target_pose, vel, accel)
                        rtde_c.servoStop()
                        assert rtde_c.moveL(target_pose, vel, accel)
